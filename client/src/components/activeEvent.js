...
import { ACTIVEEVENT_ROUTE, ARCHIVEDEVENT_ROUTE, CREATEEVENT_ROUTE, EVENT_ROUTE, MAIN_ROUTE, PROFILEORG_ROUTE } from '../utils/consts';
import style from '../style/userprofile.module.css';
import { Context } from '..';
import logo from '../image/logo.png';

const ActiveEvent = observer(() => {
  const { user, organization, event } = useContext(Context);
  const navigate = useNavigate();

  useEffect(() => {
    if (organization.organization.id) {
      event.loadActiveEvents(organization.organization.id);
    }
